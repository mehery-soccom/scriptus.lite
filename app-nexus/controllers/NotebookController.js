...
const qapairs = z.object({
  ...
  topic_id: z.string({ required_error: "topic_id is required." }).min(1, { message: "topic_id cannot be empty." }),
  ques: z
...
async function deleteQaDocs(ids, tenant_partition_key, kb_id,topic_id) {
  const filter = `tenant_partition_key == "${tenant_partition_key}" AND kb_id == "${kb_id}" AND topic_id == "${topic_id}" AND article_id in ${JSON.stringify(ids)}`;
  const resVectorDb = await deleteQaDocsVectorDb(filter);
  const mongodeleteResult = await deleteKbqaDocs(ids, tenant_partition_key, kb_id,topic_id);
...
export default class NotebookController {
  ...
  async deleteQaPairs({ request }) {
    ...
    // const filter = `tenant_partition_key == "${tenant_partition_key}"`;
    const delRes = await deleteQaDocs(ids, tenant_partition_key, kb_id, topic_id);
  ...
  async getQaPairsMongodb({ request }) {
    ...
    const paginatedDocs = await getPaginatedDocs(kb_id,topic_id, tenant_partition_key, lastSeenId, pageSize, page);
  ...
  async getQaPairsVectordb({ request }) {
    ...
    const output_fields = ["id", "kb_id", "topic_id", "article_id", "knowledgebase"];
    let paginationQuery = {};
    if (page <= 1) {
      paginationQuery = {
        collection_name: collection_name,
  ...
  async qaUpdate({ request }) {
    ...
    const tenant_partition_key = context.getTenant();
    const storedDocs = await fetchDocsByIds(update_ids, kb_id,topic_id, tenant_partition_key);
    const newDocs = await getDocsUpdateStatus(updateDocs, storedDocs);
    const updatedDocs = await updateQaDocs(newDocs);
    const filter = `tenant_partition_key == "${tenant_partition_key}" AND kb_id == "${kb_id}" AND topic_id == "${topic_id}" AND article_id in ${JSON.stringify(update_ids)}`;
  ...
  @RequestMapping({ path: "/api/qapairs/topic", method: "post" })
  @ResponseBody
  async createTopicByKb({ request }) {
    const body = request.body;
    const topic_name = body.topic_name;
    const kb_id = body.kb_id;
    const tenant_partition_key = context.getTenant();
    const result = await createNewTopic(kb_id,topic_name,tenant_partition_key);
    return {result };
  }
  @RequestMapping({ path: "/api/qapairs/topic", method: "get" })
  @ResponseBody
  async getTopicsByKb({ request }) {
    const query = request.query;
    const isDetailed = query.isDetailed;
    const kb_id = query.kb_id;
    const tenant_partition_key = context.getTenant();
    const result = await getAllTopicsByKb(kb_id,tenant_partition_key,isDetailed);
    return { result };
  }
  @RequestMapping({ path: "/api/qapairs/topic", method: "delete" })
  @ResponseBody
  async deleteTopicByKb({ request }) {
    const tenant_partition_key = context.getTenant();
    const body = request.body;
    const kb_name = body.kb_name;
    const kb_id = body.kb_id;
    const topic_name = body.topic_name;
    const topic_id = body.topic_id;
    const resultMongo = await deleteTopicByKb(kb_id, topic_id, tenant_partition_key);
    const filter = `tenant_partition_key == "${tenant_partition_key}" AND topic_id == "${topic_id}"`;
    const resultVectordb = await deleteQaDocsVectorDb(filter);
    return {  success : true, resultMongo , resultVectordb };
  }
