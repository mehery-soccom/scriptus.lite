...
function normalizePath(path) {
  ...
  export function loadControllers(app) {
    const controllersPath = join(process.cwd(), "app/controllers");
    ...
    for (const file of controllerFiles) {
      ...
      if(!controller._routed){
        ...
        for( const {path,method,handler, responseType, name} of controller.maps){
          let full_path = normalizePath(`/${controller.basePath}${path}`);
          console.log(`@RequestMappings:${method}:/${full_path} -> ${name}`)
          app[method](`${full_path}`, async (req, res) => {
            try {
              const CONST = {
                CDN_URL : "https://deployed.boot-web.pages.dev", CDN_DEBUG :  false,
                APP_TITLE : "Test",
              ...
              const result = await handler.call(cTarget, {
                  request : req, response : res, model, CONST
              });
